...
test('customer catalogue uses the public shop API and never staff products', () => {
  ...
  assert.match(home, /selectHeroProduct/)
  assert.match(home, /heroProduct\.image/)
  assert.match(home, /cube-outline/)
...
test('home hero uses the public catalogue image and search hides counts until a query', () => {
  ...
  assert.match(merch, /export function selectHeroProduct/)
  assert.match(merch, /item\.featured && item\.inStock && Boolean\(item\.image\)/)
  assert.match(home, /source=\{\{ uri: heroProduct\.image \}\}/)
