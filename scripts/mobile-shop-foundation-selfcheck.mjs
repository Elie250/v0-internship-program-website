...
test('catalogue errors distinguish network from HTTP failures', () => {
  ...
  assert.match(helper, /catalogue.timeout/)
  assert.match(helper, /not_found/)
  assert.match(helper, /invalid_json/)
  assert.match(helper, /invalid_payload/)
  assert.match(helper, /catalogue.unauthorized/)
  assert.match(helper, /catalogue.forbidden/)
...
test('home hero uses the public catalogue image and search hides counts until a query', () => {
  ...
  assert.match(hero, /source=\{\{ uri: currentImage \}\}/)
