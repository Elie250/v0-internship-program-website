...
test('8 MoMo proof is required before submit', () => {
  ...
  assert.match(readMobile('src/api/public.ts'), /body.append\('file'/)
  assert.doesNotMatch(readMobile('src/api/public.ts'), /as unknown as Blob/)
  assert.match(readMobile('src/api/client.ts'), /expo\/fetch/)
  assert.match(screen, /copyReceiptForUpload/)
