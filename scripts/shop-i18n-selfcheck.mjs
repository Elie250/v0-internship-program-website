...
test('approved storefront Kinyarwanda vocabulary is present', () => {
  ...
  assert.match(rw, /'storefront\.header\.cart': 'Ibyatoranijwe \(\{n\}\)'/)
  assert.match(rw, /'storefront\.header\.download': 'Manura App'/)
  ...
  assert.match(rw, /'storefront\.latest\.title': 'Ibicuruzwa bya vuba'/)
  assert.match(rw, /'storefront\.trends\.title': 'Ibicuruzwa byo kureba'/)
...
test('selling-unit labels stay bilingual without translating unit abbreviations', () => {
  const en = extractShopMessages(read('lib/shop/i18n/messages/en.ts'))
  const rw = extractShopMessages(read('lib/shop/i18n/messages/rw.ts'))
  assert.equal(en.get('storefront.line.unitQty'), '{unit} · Qty {n}')
  assert.equal(rw.get('storefront.line.unitQty'), '{unit} · Ingano {n}')
  assert.equal(en.get('pos.line.qtyUnit'), 'Qty {n} · {unit}')
  assert.equal(rw.get('pos.line.qtyUnit'), 'Ingano {n} · {unit}')
  assert.equal(en.get('products.field.sellingQuantity'), 'Selling quantity')
  ...
  assert.match(helper, /'PCS',/)
  assert.match(helper, /'ML',/)
})
