...
test('successful confirmation uses required English and Kinyarwanda wording', () => {
  ...
  assert.match(ui, /storefront\.checkout\.receiptEmail/)
...
test('marketplace sections stay distinct and do not duplicate chrome', () => {
  ...
  assert.match(merchUi, /storefront\.popular\.title/)
  assert.match(merchUi, /lg:grid-cols-\[minmax\(26rem,50%\)_minmax\(0,1fr\)\]/)
  assert.match(merchUi, /lg:items-stretch/)
  assert.match(merchUi, /lg:h-\[50vh\]/)
  assert.doesNotMatch(merchUi, /min-h-\[360px\]/)
  assert.match(merchUi, /lg:hidden/)
...
test('hero is a full-bleed carousel of real catalogue products', () => {
  ...
  assert.match(home, /w-\[78%\]/)
  assert.match(home, /w-\[56%\]/)
  assert.match(home, /shop-hero-fade\.png/)
  assert.match(home, /text-white/)
  assert.match(home, /text-\[#1FA64A\]/)
  assert.match(home, /text-\[#D1D5DB\]/)
  assert.match(home, /lg:text-4xl/)
  assert.match(home, /lg:text-5xl/)
  assert.match(home, /h-\[216px\]/)
  assert.match(home, /lg:h-\[50vh\]/)
  assert.doesNotMatch(home, /lg:h-full|lg:min-h-\[360px\]/)
