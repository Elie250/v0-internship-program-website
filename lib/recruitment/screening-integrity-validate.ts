/**
 * Validate and sanitize screening integrity events.
...
export function rejectClientControlledIntegrityFields(body: Record<string, unknown>): void {
  // Callers must ignore these; helper documents the invariant for tests.
...
export function sanitizePayload(raw: unknown): {
  ...
  for (const [key, value] of Object.entries(src)) {
    ...
    if (key === 'technical_score' || key === 'technicalScore') continue
    ...
    // Drop nested objects/arrays to avoid PII dumps
  }
  return { payload }
}
...
export function normalizeEventType(raw: string): IntegrityEventType | null {
  ...
  // Aliases from Phase 4 candidate UI
...
export function normalizeClientEventAt(
  ...
  // Diagnostic only — never used for integrity timeline ordering
...
/** Candidates must never read integrity assessments via candidate APIs. */
export function candidateMayReadIntegrityBand(): boolean {
  return false
}

/** Integrity APIs must never accept client-supplied technical scores. */
export function integrityApiMayModifyTechnicalScore(): boolean {
  return false
}

/** Integrity band cannot be set by the browser. */
