...
import { runInBackground } from "@/lib/async/background-task";
...
export async function createCommerceSale(
  ...
  const orderPayloadBase = {
    ...
    delivery_address:
      fulfillmentType === "delivery"
        ? String(input.deliveryAddress ?? "").trim()
        : null,
    notes: input.notes?.trim() || (input.channel === "pos" ? "POS sale" : null),
  ...
  if (
    ...
    console.warn(
      "[commerce] orders.location_id unavailable — inserting without location",
    );
  ...
  if (isPaidNow) {
    runInBackground(async () => {
      ...
      await sendConfirmedOrderReceiptByOrderId(order.id);
    }, "shop_order_receipt");
  }
