/**
 * English source dictionary for Energy & Logics Shop UI.
 * Keys are stable; do not rename without updating callers.
 */
export const shopMessagesEn = {
  // Brand
  'brand.name': 'Energy & Logics Shop',
  'brand.short': 'Energy & Logics',
  'brand.siteLabel': 'Nyanza Shop',
  'a11y.openMenu': 'Open menu',
  'a11y.nav': 'Shop navigation',
  'a11y.language': 'Language',

  // Navigation
  'nav.dashboard': 'Dashboard',
  'nav.dashboardDesc': 'Overview of shop operations',
  'nav.orders': 'Orders',
  'nav.ordersDesc': 'Customer orders awaiting action',
  'nav.pos': 'POS',
  'nav.posDesc': 'Point of sale terminal',
  'nav.products': 'Products',
  'nav.productsDesc': 'Product catalog',
  'nav.inventory': 'Inventory',
  'nav.inventoryDesc': 'Stock levels and movements',
  'nav.sales': 'Sales',
  'nav.salesDesc': 'Sales and order history',
  'nav.staff': 'Staff',
  'nav.staffDesc': 'Manage shop staff accounts',
  'nav.settings': 'Settings',
  'nav.settingsDesc': 'Account and shop preferences',

  // Roles
  'role.admin': 'Administrator',
  'role.salesperson': 'Salesperson',
  'role.inventory_manager': 'Inventory Manager',
  'role.support_staff': 'Support staff',
  'role.engineer': 'Engineer',

  // Common actions
  'action.signIn': 'Sign in',
  'action.signingIn': 'Signing in…',
  'action.signOut': 'Sign out',
  'action.signingOut': 'Signing out…',
  'action.search': 'Search',
  'action.add': 'Add',
  'action.edit': 'Edit',
  'action.delete': 'Delete',
  'action.save': 'Save',
  'action.saveChanges': 'Save changes',
  'action.saving': 'Saving…',
  'action.cancel': 'Cancel',
  'action.close': 'Close',
  'action.confirm': 'Confirm',
  'action.create': 'Create',
  'action.view': 'View',
  'action.details': 'Details',
  'action.refresh': 'Refresh',
  'action.back': 'Back',
  'action.previous': 'Previous',
  'action.next': 'Next',

  // Common labels
  'common.language': 'Language',
  'common.status': 'Status',
  'common.price': 'Price',
  'common.costPrice': 'Cost price',
  'common.sellingPrice': 'Selling price',
  'common.quantity': 'Quantity',
  'common.total': 'Total',
  'common.discount': 'Discount',
  'common.payment': 'Payment',
  'common.customer': 'Customer',
  'common.customers': 'Customers',
  'common.location': 'Location',
  'common.shop': 'Shop',
  'common.role': 'Role',
  'common.permission': 'Permission',
  'common.receipt': 'Receipt',
  'common.stock': 'Stock',
  'common.product': 'Product',
  'common.products': 'Products',
  'common.sale': 'Sale',
  'common.sales': 'Sales',
  'common.inventory': 'Inventory',
  'common.movement': 'Movement',
  'common.stockMovement': 'Stock movement',
  'common.pos': 'POS',
  'common.staff': 'Staff',
  'common.loading': 'Loading…',
  'common.emDash': '—',
  'common.sku': 'SKU',
  'common.published': 'Published',
  'common.draft': 'Draft',
  'common.archived': 'Archived',
  'common.paid': 'Paid',
  'common.pending': 'Pending',
  'common.approved': 'Approved',
  'common.cash': 'Cash',
  'common.completed': 'Completed',
  'common.available': 'Available',
  'common.subtotal': 'Subtotal',
  'common.order': 'Order',
  'common.orders': 'Orders',
  'stockState.consumed': 'Consumed',
  'stockState.reserved': 'Reserved',
  'stockState.released': 'Released',

  // Pagination
  'pagination.none': 'No results',
  'pagination.showing': 'Showing {from}–{to} of {total}',
  'pagination.page': 'Page {page} / {totalPages}',

  // Auth
  'auth.title': 'Shop staff sign in',
  'auth.subtitle':
    'Sign in with your Energy & Logics staff account to access POS and inventory.',
  'auth.email': 'Email',
  'auth.password': 'Password',
  'auth.error.generic': 'Unable to sign in. Check your email and password and try again.',
  'auth.error.retry': 'Unable to sign in. Please try again.',

  // Forbidden
  'forbidden.title': 'Access restricted',
  'forbidden.body':
    'Your account is signed in, but it does not have permission for this section. Ask an administrator if you need access.',

  // Errors
  'error.requestFailed': 'Something went wrong. Please try again.',
  'error.network': 'Connection problem. Check your network and try again.',

  // Dashboard
  'dashboard.title': 'Dashboard',
  'dashboard.description':
    'Operational overview for the Nyanza shop. Figures are based on recorded shop activity.',
  'dashboard.unavailable': 'Dashboard unavailable',
  'dashboard.noPerms':
    'You are signed in to {brandName}, but your role cannot view sales or inventory reports. Ask an administrator if you need access.',
  'dashboard.noData': 'No dashboard figures are available yet.',
  'dashboard.metric.todaySales': "Today's sales",
  'dashboard.metric.todaySalesHint': 'Paid / approved orders only',
  'dashboard.metric.todayOrders': "Today's orders",
  'dashboard.metric.todayOrdersHint': '{pos} POS · {online} online',
  'dashboard.metric.pending': 'Pending payments',
  'dashboard.metric.pendingHint': 'Unpaid, awaiting review, or MoMo pending',
  'dashboard.metric.catalog': 'Catalog items',
  'dashboard.metric.catalogHint': 'Published and draft products',
  'dashboard.metric.inStock': 'In stock',
  'dashboard.metric.lowStock': 'Low stock',
  'dashboard.metric.lowStockHint': 'At or below threshold, still available',
  'dashboard.metric.outOfStock': 'Out of stock',
  'dashboard.businessDay': 'Business day',
  'dashboard.stockModelNote': 'Stock figures for all products.',
  'dashboard.section.salesToday': 'Sales today',
  'dashboard.section.salesTodayDesc':
    'Sales figures are based on recorded shop transactions.',
  'dashboard.profitNote':
    'Profit reporting is not enabled yet.',
  'dashboard.section.inventory': 'Inventory snapshot',
  'dashboard.section.inventoryDesc':
    'Stock figures for all products.',
  'dashboard.section.shortcuts': 'Shortcuts',
  'dashboard.shortcut.pos': 'Open POS',
  'dashboard.shortcut.sales': 'Sales history',
  'dashboard.shortcut.inventory': 'Inventory',

  // POS
  'pos.title': 'POS',
  'pos.description':
    'Cash point of sale. Look up products, take payment, and print a receipt.',
  'pos.descriptionForbidden': 'Point of sale terminal.',
  'pos.defaultCustomer': 'Walk-in customer',
  'pos.searchLabel': 'Search products',
  'pos.searchPlaceholder': 'Search by name, SKU, or barcode…',
  'pos.searching': 'Searching…',
  'pos.catalogHint': 'Published products from the shop catalog',
  'pos.error.loadProducts': 'Unable to show products. Please try again.',
  'pos.noSku': 'No SKU',
  'pos.stockLabel': 'Stock {n}',
  'pos.emptyProducts': 'No products found. Try another search.',
  'pos.cartTitle': 'Current Sale',
  'pos.cartHint':
    'These are preview totals. The final total will be shown when the sale is confirmed.',
  'pos.cartEmpty': 'Select products to build the cart.',
  'pos.items': 'Items',
  'pos.quantity': 'Quantity',
  'pos.unitPrice': 'Unit Price',
  'pos.lineTotal': 'Total',
  'pos.remove': 'Remove',
  'pos.openCart': 'View current sale',
  'pos.itemCount': '{n} item',
  'pos.itemCountPlural': '{n} items',
  'pos.shop': 'Shop',
  'pos.reviewTitle': 'Review sale',
  'pos.each': '{price} each',
  'pos.offList': '· −{discount} off list',
  'pos.line.qtyUnit': 'Qty {n} · {unit}',
  'a11y.decreaseQty': 'Decrease quantity',
  'a11y.increaseQty': 'Increase quantity',
  'a11y.removeItem': 'Remove',
  'pos.listSubtotal': 'Subtotal',
  'pos.discounts': 'Discount',
  'pos.previewTotal': 'Total',
  'pos.customerName': 'Customer',
  'pos.phoneOptional': 'Phone (optional)',
  'pos.paymentCash': 'Payment: Cash',
  'pos.paymentNote':
    'MoMo is not available yet. Stock is updated when the sale is confirmed.',
  'pos.reviewSale': 'Review Sale',
  'pos.confirmPrompt': 'Confirm cash payment of {total}?',
  'pos.confirmHint':
    'Confirming records the sale and updates stock.',
  'pos.confirmSale': 'Confirm Cash Sale',
  'pos.processing': 'Processing…',
  'pos.saleFailed': 'Sale failed',
  'pos.successTitle': 'Sale completed',
  'pos.idempotentReplay': 'This sale was already recorded.',
  'pos.serverTotal': 'Total',
  'pos.cashDotStatus': 'Cash · {paymentStatus}',
  'pos.newSale': 'New Cash Sale',
  'pos.successBody': 'Sale completed and stock updated.',
  'pos.thankYou': 'Thank you for shopping at Energy & Logics.',

  // Products
  'products.title': 'Products',
  'products.description':
    'Browse the Energy & Logics catalog. Product information is available in the shop system.',
  'products.descriptionForbidden': 'Product catalog.',
  'products.readOnlyNote':
    'Product information from the shop system. Editing is not available yet.',
  'products.manageNote':
    'Create, edit, and archive products like the admin catalog. Inventory managers have this by default; other staff need the admin grant “Create / edit / archive products”. Price fields still require cost or selling-price permission. New stock starts at 0 — receive stock after create.',
  'products.create': 'Create product',
  'products.edit': 'Edit product',
  'products.field.name': 'Name',
  'products.field.description': 'Description',
  'products.field.image': 'Photo',
  'products.barcodeHint': 'Letters and numbers only. Scan or type. Leave blank if there is none.',
  'products.stockCreateHint': 'Stock starts at 0. Receive units from Inventory after you create the product. Do not type an on-hand count here.',
  'products.createdTitle': 'Product created. Stock is 0.',
  'products.createdBody': 'Receive stock next so the movement is recorded.',
  'products.receiveStock': 'Receive stock',
  'products.done': 'Done',
  'products.field.status': 'Status',
  'products.field.lowStockThreshold': 'Low-stock threshold',
  'products.field.targetStock': 'Target stock',
  'products.lowStockHint':
    'A product is considered low stock when current stock is at or below this value.',
  'products.targetStockHint':
    'Used to calculate the suggested replenishment quantity. This is not the current on-hand count.',
  'products.archive': 'Archive',
  'products.archived': 'Product archived.',
  'products.publish': 'Set published',
  'products.unarchive': 'Restore as published',
  'products.searchLabel': 'Search products',
  'products.searchPlaceholder': 'Search name, SKU, barcode…',
  'products.statusLabel': 'Status',
  'products.status.all': 'All statuses',
  'products.col.product': 'Product',
  'products.col.price': 'Price',
  'products.col.stock': 'Stock',
  'products.col.status': 'Status',
  'products.noSku': 'No SKU',
  'products.empty': 'No products found.',
  'products.selectHint': 'Select a product to view details.',
  'products.field.sku': 'SKU',
  'products.field.barcode': 'Barcode',
  'products.field.category': 'Category',
  'products.field.listPrice': 'List price',
  'products.field.discount': 'Discount',
  'products.field.sellPrice': 'Sell price',
  'products.field.cost': 'Cost',
  'products.field.stock': 'Stock',
  'products.field.sellingQuantity': 'Selling quantity',
  'products.field.sellingUnit': 'Selling unit',
  'products.field.featured': 'Featured on storefront',
  'products.featuredHint':
    'Featured products appear first in the New Arrivals carousel when they have a photo and are in stock.',
  'products.featuredOn': 'Yes',
  'products.featuredOff': 'No',
  'products.sellingHint':
    'Price applies to one selling unit. Stock is the number of selling units.',
  'products.sellingSaved': 'Selling unit saved.',
  'products.sellingInvalid': 'Enter a selling quantity greater than 0 and choose a listed unit.',
  'products.refreshing': 'Refreshing…',

  // Inventory
  'inventory.title': 'Inventory',
  'inventory.description':
    'Select a product to receive stock or adjust quantity. Transfers are not available.',
  'inventory.descriptionForbidden': 'Stock levels and movements.',
  'inventory.readOnlyNote':
    'Select a product to receive stock or adjust quantity. Receive adds units from a supplier or source. Adjust corrects the current quantity with a signed change. Transfers are not available.',
  'inventory.selectHint': 'Select a product to receive stock or adjust quantity.',
  'inventory.receiveExplain':
    'Receive adds stock received from a supplier or source. Example: current 10, receive +5, new stock 15.',
  'inventory.adjustExplain':
    'Adjust uses a signed change. Example: current 10, adjustment −2, new stock 8. This is not a new total.',
  'inventory.preview': 'New stock: {n}',
  'inventory.success.receive': 'Stock received successfully. {delta} units. New stock: {n}.',
  'inventory.success.adjust': 'Stock adjusted successfully. {delta} units. New stock: {n}.',
  'inventory.error.qtyReceive': 'Enter how many units to receive.',
  'inventory.error.qtyAdjust': 'Enter a signed quantity change, for example 5 or -2.',
  'inventory.error.reason': 'Enter a reason for this stock movement.',
  'inventory.tab.levels': 'Stock levels',
  'inventory.tab.movements': 'Movements',
  'inventory.tab.replenishment': 'Replenishment',
  'inventory.action.adjust': 'Adjust stock',
  'inventory.action.receive': 'Receive stock',
  'inventory.action.request': 'Request',
  'inventory.field.qty': 'Quantity',
  'inventory.field.receiveQty': 'Quantity to receive',
  'inventory.field.adjustQty': 'Signed quantity change',
  'inventory.field.reason': 'Reason',
  'inventory.searchPlaceholder': 'Search name, SKU, or barcode…',
  'inventory.col.target': 'Target',
  'inventory.col.onOrder': 'On order',
  'inventory.col.suggested': 'Suggested',
  'inventory.emptyReplenishment': 'No products are at or below the reorder point.',
  'inventory.searchLabel': 'Search inventory',
  'inventory.movementTypeLabel': 'Movement type',
  'inventory.movementType.all': 'All movement types',
  'inventory.col.product': 'Product',
  'inventory.col.onHand': 'On hand',
  'inventory.col.threshold': 'Threshold',
  'inventory.col.listPrice': 'List price',
  'inventory.col.flag': 'Flag',
  'inventory.flag.out': 'Out of stock',
  'inventory.flag.low': 'Low stock',
  'inventory.flag.ok': 'OK',
  'inventory.emptyLevels': 'No stock figures.',
  'inventory.col.when': 'When',
  'inventory.col.type': 'Type',
  'inventory.col.delta': 'Change',
  'inventory.col.beforeAfter': 'Before → After',
  'inventory.col.reason': 'Reason',
  'inventory.emptyMovements': 'No movements found.',

  // Sales
  'sales.title': 'Sales',
  'sales.description': 'POS and online sales history.',
  'sales.descriptionForbidden': 'Sales history.',
  'sales.readOnlyNote':
    'Sales history from the shop system. Edits and refunds are not available here.',
  'sales.channelLabel': 'Channel',
  'sales.channel.all': 'All channels',
  'sales.channel.pos': 'POS',
  'sales.channel.online': 'Online',
  'sales.paymentStatusLabel': 'Payment status',
  'sales.payment.all': 'All payment statuses',
  'sales.payment.paid': 'Paid',
  'sales.payment.unpaid': 'Unpaid',
  'sales.payment.pendingReview': 'Pending review',
  'sales.payment.gatewayPending': 'MoMo pending',
  'sales.payment.approved': 'Approved',
  'sales.col.order': 'Order',
  'sales.col.channel': 'Channel',
  'sales.col.payment': 'Payment',
  'sales.col.total': 'Total',
  'sales.empty': 'No sales found.',
  'sales.selectHint': 'Select a sale to view details.',
  'sales.orderFallbackTitle': 'Order',
  'sales.field.total': 'Total',
  'sales.field.customer': 'Customer',
  'sales.field.location': 'Location',
  'sales.field.payment': 'Payment',
  'sales.field.stockState': 'Stock',
  'sales.lineCost': '· cost {amount}',

  // Operational orders queue
  'orders.title': 'Orders',
  'orders.description': 'Review customer MoMo payments and manage order fulfillment.',
  'orders.descriptionForbidden': 'Customer orders awaiting action.',
  'orders.pendingTitle': 'Pending Orders',
  'orders.paymentAwaiting': 'Payment awaiting confirmation',
  'orders.momoPayment': 'MoMo Payment',
  'orders.paymentProof': 'Payment Proof',
  'orders.approvePayment': 'Approve Payment',
  'orders.rejectPayment': 'Reject Payment',
  'orders.phone': 'Phone',
  'orders.email': 'Email',
  'orders.paymentReference': 'Payment Reference',
  'orders.reviewedBy': 'Reviewed By',
  'orders.reviewedAt': 'Reviewed At',
  'orders.confirmed': 'Order Confirmed',
  'orders.paymentRejected': 'Payment Rejected',
  'orders.readyForPickup': 'Ready for Pickup',
  'orders.outForDelivery': 'Out for Delivery',
  'orders.completed': 'Completed',
  'orders.notesLabel': 'Notes',
  'orders.notesPlaceholder': 'Optional notes for the customer',
  'orders.fulfillment': 'Fulfillment',
  'orders.updateStatus': 'Update status',
  'orders.openProof': 'View payment proof',
  'orders.noPending': 'No payments waiting for confirmation.',
  'orders.empty': 'No orders found.',
  'orders.selectHint': 'Select an order to review payment and fulfillment.',
  'orders.submittedAt': 'Submitted',
  'orders.amount': 'Amount',
  'orders.items': 'Items',
  'orders.sellingUnit': 'Selling unit',
  'orders.unitPrice': 'Unit price',
  'orders.approving': 'Approving…',
  'orders.rejecting': 'Rejecting…',
  'orders.updating': 'Updating…',
  'orders.fulfillmentPaidOnly': 'Fulfillment is available after payment is approved.',
  'orders.onlineQueueNote': 'Online customer orders. POS history remains under Sales.',

  // Staff
  'staff.title': 'Staff Management',
  'staff.description':
    'Create salesperson and inventory manager accounts, reset passwords, and revoke Shop sessions. Administrators cannot be created here.',
  'staff.descriptionForbidden':
    'Create and manage salesperson and inventory manager accounts.',
  'staff.count': '{n} staff member',
  'staff.countPlural': '{n} staff members',
  'staff.create': 'Create Staff',
  'staff.searchLabel': 'Search staff',
  'staff.searchPlaceholder': 'Search name or email…',
  'staff.filter.role': 'Filter by role',
  'staff.filter.role.all': 'All roles',
  'staff.filter.status': 'Filter by status',
  'staff.filter.status.all': 'All statuses',
  'staff.filter.status.active': 'Active',
  'staff.filter.status.inactive': 'Inactive',
  'staff.filter.status.suspended': 'Suspended',
  'staff.loading': 'Loading staff…',
  'staff.emptyTitle': 'No staff found',
  'staff.emptyBody': 'Create a salesperson or inventory manager to get started.',
  'staff.col.name': 'Name',
  'staff.col.email': 'Email',
  'staff.col.role': 'Role',
  'staff.col.status': 'Status',
  'staff.col.created': 'Created',
  'staff.col.lastSession': 'Last session',
  'staff.col.actions': 'Actions',
  'staff.you': '(you)',
  'staff.activeSessions': '({n} active)',
  'staff.createdPrefix': 'Created {date}',
  'staff.action.resetPassword': 'Reset password',
  'staff.action.revokeSessions': 'Revoke sessions',
  'staff.success.created': 'Staff account created.',
  'staff.success.updated': 'Staff account updated.',
  'staff.success.passwordReset':
    'Password reset. Active sessions for this user were revoked.',
  'staff.success.sessionsRevoked': 'Sessions revoked.',
  'staff.create.title': 'Create staff',
  'staff.create.desc':
    'Create a salesperson or inventory manager. Administrators cannot be created here.',
  'staff.field.firstName': 'First name',
  'staff.field.lastName': 'Last name',
  'staff.field.email': 'Email',
  'staff.field.role': 'Role',
  'staff.field.password': 'Password',
  'staff.field.confirmPassword': 'Confirm password',
  'staff.field.status': 'Status',
  'staff.field.newPassword': 'New password',
  'staff.create.submit': 'Create staff',
  'staff.create.submitting': 'Creating…',
  'staff.validation.fullName': 'Full name is required',
  'staff.validation.email': 'Email is required',
  'staff.validation.role': 'Role must be salesperson or inventory manager',
  'staff.validation.passwordLength': 'Password must be at least 6 characters',
  'staff.validation.passwordMatch': 'Passwords do not match',
  'staff.edit.title': 'Edit staff',
  'staff.edit.desc':
    'Update profile and status. Role changes for administrators are not available here.',
  'staff.edit.roleReadonly': 'Role: {roleLabel}',
  'staff.status.active': 'Active',
  'staff.status.inactive': 'Inactive',
  'staff.edit.save': 'Save changes',
  'staff.edit.saving': 'Saving…',
  'staff.reset.title': 'Reset password',
  'staff.reset.desc':
    'Set a new password for {name}. Active Shop sessions for this account will be revoked.',
  'staff.reset.descFallbackName': 'this staff member',
  'staff.reset.submit': 'Reset password',
  'staff.reset.resetting': 'Resetting…',
  'staff.revoke.title': 'Revoke sessions?',
  'staff.revoke.desc':
    'This will sign {name} out of active Shop sessions. They will need to sign in again.',
  'staff.revoke.confirm': 'Revoke sessions',
  'staff.perms.title': 'Shop permissions',
  'staff.perms.hint':
    'Role defaults stay checked. Extra boxes grant temporary or additional access after the staff member signs in again.',
  'staff.perms.fromRole': 'Included in role',
  'staff.perms.group.product': 'Product management',
  'staff.perms.group.pos': 'POS and sales',
  'staff.perms.group.inventory': 'Inventory',
  'staff.perms.group.pricing': 'Pricing',
  'staff.perms.group.replenishment': 'Replenishment',
  'staff.perms.products': 'Create / edit / archive products',
  'staff.perms.productsView': 'Product view',
  'staff.perms.pos': 'POS',
  'staff.perms.salesView': 'Sales history',
  'staff.perms.ordersView': 'View orders',
  'staff.perms.ordersManage': 'Manage orders',
  'staff.perms.stockView': 'Inventory view',
  'staff.perms.stockAdjust': 'Stock adjust',
  'staff.perms.stockReceive': 'Stock receive',
  'staff.perms.costPrice': 'Edit cost price',
  'staff.perms.sellingPrice': 'Edit selling price',
  'staff.perms.replenishmentView': 'View low-stock / replenishment',
  'staff.perms.purchaseRequest': 'Create purchase request',

  // Settings
  'settings.title': 'Settings',
  'settings.description':
    'Account and shop preferences. Advanced configuration will be added later.',
  'settings.account.title': 'Signed-in account',
  'settings.account.phaseHint':
    'Use Sign out in the sidebar or header to end your session',
  'settings.context.title': 'Operational context',
  'settings.context.body':
    '{brandName} — {siteLabel}. This display name does not change stock figures.',
  'settings.context.phaseHint':
    'Shop location is managed separately.',
  'settings.prefs.title': 'Shop preferences',
  'settings.prefs.body':
    'User management, payment configuration, and EBM settings are not available yet.',
  'settings.prefs.phaseHint': 'Coming later',

  // Public storefront
  'storefront.nav.products': 'Products',
  'storefront.nav.cart': 'Cart',
  'storefront.nav.track': 'Track Order',
  'storefront.staff': 'Staff',
  'storefront.shoppingFrom': 'Shopping from',
  'storefront.header.download': 'Download App',
  'storefront.header.downloadSoon': 'Coming soon',
  'storefront.header.cart': 'Cart ({n})',
  'storefront.a11y.nav': 'Shop navigation',
  'storefront.a11y.cart': 'Cart',
  'storefront.hero.eyebrow': 'Electronics and accessories',
  'storefront.hero.title': 'Quality Electronics',
  'storefront.hero.emphasis': 'Everyday',
  'storefront.hero.body': 'Components, tools, devices & more.',
  'storefront.hero.shopNow': 'Shop now',
  'storefront.hero.browse': 'Browse products',
  'storefront.seeAll': 'See all',
  'storefront.hero.viewProduct': 'View product',
  'storefront.hero.nowAt': 'Available now at Nyanza Shop.',
  'storefront.hero.previous': 'Previous product',
  'storefront.hero.next': 'Next product',
  'storefront.hero.goto': 'Show product {n}',
  'storefront.hero.carousel': 'Featured products',
  'storefront.arrivals.title': 'New Arrivals',
  'storefront.arrivals.subtitle': 'Recently added to Nyanza Shop.',
  'storefront.latest.title': 'Latest Arrivals',
  'storefront.latest.subtitle': 'Recently added products you can buy now.',
  'storefront.trends.title': 'Trending now',
  'storefront.trends.subtitle': 'Products to explore at Nyanza Shop.',
  'storefront.popular.title': 'Popular',
  'storefront.featured.title': 'Featured products',
  'storefront.featured.subtitle': 'Highlighted from products we sell at Nyanza Shop.',
  'storefront.promo.sound.title': 'Premium Sound',
  'storefront.promo.sound.body': 'Audio products available in the shop.',
  'storefront.promo.power.title': 'Stay Powered',
  'storefront.promo.power.body': 'Chargers, batteries and power accessories.',
  'storefront.promo.shop': 'Shop this range',
  'storefront.categories.title': 'Categories',
  'storefront.deals.title': "Today's Deals",
  'storefront.deals.subtitle': 'Products with a reduced price at Nyanza Shop.',
  'storefront.more.title': 'More in shop',
  'storefront.catalogue.title': 'Products',
  'storefront.catalogue.hint':
    'Phones, audio, chargers, batteries and electronics accessories.',
  'storefront.catalogue.soon': 'The product list will appear here shortly.',
  'storefront.catalogue.searchPlaceholder': 'Search products',
  'storefront.catalogue.all': 'All',
  'storefront.catalogue.empty': 'No products are listed right now.',
  'storefront.catalogue.noResults': 'No products match that search.',
  'storefront.catalogue.error':
    'The product list could not be loaded. Please try again shortly.',
  'storefront.catalogue.addToCart': 'Add to cart',
  'storefront.catalogue.unavailable': 'Unavailable',
  'storefront.catalogue.loading': 'Loading products…',
  'storefront.availability.available': 'Available',
  'storefront.availability.few': 'Few left',
  'storefront.availability.out': 'Out of stock',
  'storefront.product.notFound': "Sorry, we couldn't find that product.",
  'storefront.product.back': 'Back to products',
  'storefront.cart.title': 'Cart',
  'storefront.cart.empty': 'Your cart is empty.',
  'storefront.cart.continue': 'Continue shopping',
  'storefront.line.unitQty': '{unit} · Qty {n}',
  'storefront.line.qtyOnly': 'Qty {n}',
  'storefront.cart.checkoutSoon': 'Checkout will be available shortly.',
  'storefront.checkout.title': 'Checkout',
  'storefront.checkout.soon':
    'Checkout will be available shortly. You can keep browsing products in the meantime.',
  'storefront.checkout.contact': 'Your details',
  'storefront.checkout.fullName': 'Full name',
  'storefront.checkout.phone': 'Phone',
  'storefront.checkout.fulfillment': 'How you will receive your order',
  'storefront.checkout.pickup': 'Pickup',
  'storefront.checkout.delivery': 'Delivery — we will contact you',
  'storefront.checkout.deliveryAddress': 'Delivery address',
  'storefront.checkout.notes': 'Notes (optional)',
  'storefront.checkout.continuePayment': 'Continue to payment',
  'storefront.checkout.summary': 'Order summary',
  'storefront.checkout.submit': 'Submit order',
  'storefront.checkout.submitting': 'Submitting…',
  'storefront.checkout.receiptUpload': 'MoMo payment proof',
  'storefront.checkout.receiptRef': 'Transaction reference (optional)',
  'storefront.checkout.receiptReady': 'Payment proof uploaded.',
  'storefront.checkout.successTitle': 'Order placed successfully',
  'storefront.checkout.thankYou': 'Thank you for shopping with Energy & Logics.',
  'storefront.checkout.keepNumber': 'Keep this number to track your order.',
  'storefront.checkout.cartChanged':
    'Prices or availability changed. Please review your cart.',
  'storefront.checkout.previewHint':
    'These totals are based on current prices. The final amount is confirmed when you submit.',
  'storefront.track.title': 'Track Order',
  'storefront.track.soon': 'Order tracking will be available shortly.',
  'storefront.track.heading': 'Track your order',
  'storefront.track.body':
    'Enter your order number to see your order details and status.',
  'storefront.track.orderNumber': 'Order number',
  'storefront.track.example': 'Example: EL-NYZ-20260827-0001',
  'storefront.track.notFound':
    "We couldn't find an order with that number. Please check the number and try again.",
  'storefront.track.keepNumber': 'Keep this number for future reference.',
  'storefront.track.loading': 'Looking up your order…',
  'storefront.product.title': 'Product',
  'storefront.product.soon': 'Product details will be available shortly.',
  'storefront.order.title': 'Order',
  'storefront.order.soon': 'Order confirmation will be available shortly.',
  'storefront.order.date': 'Order date',
  'storefront.order.status': 'Status',
  'storefront.order.items': 'Items',
  'storefront.order.notFoundTitle': 'Order not found',
  'storefront.order.backToTrack': 'Track an order',
  'storefront.status.received': 'Order received',
  'storefront.status.paymentAwaiting': 'Payment awaiting confirmation',
  'storefront.status.paymentConfirmed': 'Payment confirmed',
  'storefront.status.preparing': 'Preparing your order',
  'storefront.status.ready': 'Ready for pickup',
  'storefront.status.completed': 'Completed',
  'storefront.status.cancelled': 'Cancelled',
  'storefront.payment.awaiting': 'Payment awaiting confirmation',
  'storefront.payment.confirmed': 'Payment confirmed',
  'storefront.payment.notCompleted': 'Payment not completed',
  'storefront.payment.momo': 'MoMo',
  'storefront.comingSoon': 'Coming shortly',
  'storefront.footer.tagline': 'Electronics and accessories in Rwanda.',
  'storefront.footer.manage': 'Staff sign in',
} as const

export type ShopMessageKey = keyof typeof shopMessagesEn
