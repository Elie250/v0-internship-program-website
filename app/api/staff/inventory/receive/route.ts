...
export async function POST(request: Request) {
  try {
    ...
    const auth = await requireStaffPermission(request, STAFF_API_PERMISSIONS.stockReceive)
    if ('response' in auth) return auth.response

    const body = (await request.json().catch(() => ({}))) as Record<string, unknown>
    const result = await mutateStaffStock({
      productId: String(body.productId ?? body.product_id ?? ''),
      quantityDelta: Number(body.quantity ?? body.quantityDelta ?? body.quantity_delta ?? 0),
